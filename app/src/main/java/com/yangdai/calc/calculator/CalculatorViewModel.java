...
public class CalculatorViewModel extends ViewModel {
    ...
    private final MutableLiveData<String> _outputText = new MutableLiveData<>();
    ...
    public void handleEqualButton(String inputStr, Calculator formulaUtil, SharedPreferences settings, SharedPreferences history, boolean fromUser, String bigNum, String error) {
        ...
        if ("e".equals(inputStr) || "π".equals(inputStr)) {
            ...
            _outputText.setValue(inputStr);
            return;
        }

        // 忽略特殊情况
        if (inputStr.isEmpty() || isNumeric(inputStr)) {
            _outputText.setValue("");
            return;
        }
        ...
        try {
            ...
            if (fromUser) {
                ...
                editor.apply();
                _inputText.setValue(res);
                _outputText.setValue("");
            } else {
                _outputText.setValue(formatNumber(res));
            }
        } catch (Exception e) {
            if (fromUser) {
                _outputText.setValue(error);
            }
        }
    }
    ...
    public void handleInverseButton(String inputStr) {
        ...
        if (inputStr.length() > 0) {
            ...
            if (isNumber(String.valueOf(lastChar))) {
                        ...
                        if (isNumber(String.valueOf(curr)) || curr == '.') {
                            ...
                            if (curr == '-') {
                                if (i >= 1 && "(-".equals(inputStr.substring(i - 1, i + 1))) {
                                    ...
                                    _inputText.setValue(inputStr + n);
                                    left--;
                                    return;
                                }
                            }  // + × ÷ (  ^ 特殊情况 )
                            inputStr = inputStr.substring(0, i + 1);
                            String prefix = (curr == ')') ? "×(-" : "(-";
                            _inputText.setValue(inputStr + prefix + n);
                            left++;
                            return;
                        }
                    }
                ...
                //只有数字
                _inputText.setValue("(-" + n);
                left++;
                return;
            } else if (lastChar == '-') {
                // 最后是 (-， 直接去掉
                if (inputStr.length() > 1 && (inputStr.charAt(inputStr.length() - 2) == '(')) {
                    _inputText.setValue(inputStr.substring(0, inputStr.length() - 2));
                    left--;
                    return;
                }
            }
            String prefix = (lastChar == ')' || lastChar == '!') ? "×(-" : "(-";
            _inputText.setValue(inputStr + prefix);
        } else {
            _inputText.setValue("(-");
        }
        left++;
            ...
            if (inputStr.length() > 0) {
                ...
                if (isNumber(append)) {
                    if (")".equals(String.valueOf(lastInput))
                        ...
                        _inputText.setValue(inputStr + "×" + append);
                        return;
                    }
                }
                // 最后一位是两数运算符号时，再次输入符号则替换最后一位
                if (isSymbol(String.valueOf(lastInput)) && isSymbol(append)) {
                    _inputText.setValue(inputStr.substring(0, inputStr.length() - 1) + append);
                    return;
                }
                // 最后一位是数字时，输入e、π默认加上 ×
